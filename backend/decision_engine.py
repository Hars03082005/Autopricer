...
class AdaptiveRangeEngine:
    ...
    def build(self, *, prediction: float, comps: list[dict],
        ...
        if n_valid > 0 and case in ("high", "medium"):
            ...
            wlo = float((devs * sims * neg_mask).sum() / sum_sims)
            whi = float((devs * sims * pos_mask).sum() / sum_sims)
