...
DATASET      = Path(__file__).resolve().parent / "data" / "processed_s1_s4_owner_1.csv"
IS_OFFICIAL  = False
SCRIPT_NAME  = "train-3"
...
def _init_wandb(config: dict) -> None:
    ...
    if _wandb_mod is None:
        log.debug("W&B package not installed â€” skipping experiment tracking")
        return
    try:
...
def train_segment_model(seg_name, seg_df, global_model, global_cat_levels) -> tuple:
    ...
    if len(seg_df) < MIN_SEGMENT_ROWS:
        log.debug(f"Skip {seg_name} â€” only {len(seg_df)} rows")
